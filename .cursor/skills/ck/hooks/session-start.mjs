...
function gitLogSince(projectPath, sinceDate) {
  ...
  try {
    ...
    return commits > 0
      ? `${commits} commit${commits !== 1 ? 's' : ''} since last session`
      : null
  } catch {
    return null
  }
...
function main() {
    ...
    if (context) {
      ...
      const summaryLines = [
        ...
        latest.nextSteps?.length
          ? `Next: ${latest.nextSteps.slice(0, 2).join(' · ')}`
          : null,
      ].filter(Boolean)
      ...
      if (prevSession?.sessionId && prevSession.sessionId !== sessionId) {
        ...
        const alreadySaved = context.sessions?.some(
          (s) => s.id === prevSession.sessionId,
        )
        if (!alreadySaved) {
          summaryLines.push(
      ...
      parts.push(
        [`---`, `## ck: ${displayName}`, ``, summaryLines.join('\n')].join(
          '\n',
        ),
      )
  ...
  const recent = entries
    .map(([path, info]) => {
      const ctx = readJson(
        resolve(CK_HOME, 'contexts', info.contextDir, 'context.json'),
      )
