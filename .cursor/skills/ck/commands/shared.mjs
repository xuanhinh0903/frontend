...
export const SKILL_FILE = resolve(
  homedir(),
  '.claude',
  'skills',
  'ck',
...
export function gitSummary(projectPath, sinceDate) {
  ...
  const countStr = runGit(
    ['rev-list', '--count', 'HEAD', `--since=${sinceDate}`],
    projectPath,
  )
  const revCount = countStr ? parseInt(countStr, 10) : commits
  const diff = runGit(
    ['diff', '--shortstat', `HEAD~${Math.min(revCount, 50)}..HEAD`],
    projectPath,
  )
...
export function renderContextMd(ctx) {
  ...
  lines.push(
    Array.isArray(ctx.stack) ? ctx.stack.join(', ') : ctx.stack || '_Not set._',
  )
  lines.push(``)
...
export function renderBriefingBox(ctx, _meta = {}) {
  ...
  const row = (label, value) =>
    `│  ${label} → ${pad(value, W - label.length - 7)}│`
  ...
  lines.push(
    row(
      'STACK     ',
      Array.isArray(ctx.stack) ? ctx.stack.join(', ') : ctx.stack || '—',
    ),
  )
