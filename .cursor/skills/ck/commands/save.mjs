...
if (isInit) {
  ...
  while (
    existingDirs.includes(contextDir) &&
    projects[projectPath]?.contextDir !== contextDir
  ) {
  ...
  console.log(
    `  Use /ck:save to save session state and /ck:resume to reload it next time.`,
  )
  process.exit(0)
}
...
if (!context) {
  console.log(
    `ck: context.json not found for '${contextDir}'. The install may be corrupted.`,
  )
  process.exit(1)
}
...
try {
  ...
  process.stderr.write(
    `ck: warning — could not write native memory entry: ${e.message}\n`,
  )
}
