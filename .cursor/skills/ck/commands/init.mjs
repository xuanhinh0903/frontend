...
if (pkg) {
  try {
    ...
    if (parsed.description && !output.description)
...
if (claudeMd) {
  ...
  const whatItIs =
    extractSection(claudeMd, 'What This Is') ||
    extractSection(claudeMd, 'About')
  if (whatItIs && !output.description)
    output.description = whatItIs.split('\n')[0].trim()
}
