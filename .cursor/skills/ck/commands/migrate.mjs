...
for (const [projectPath, info] of Object.entries(projects)) {
  ...
  try {
    // Read v1 files
    const contextMd = existsSync(contextMdPath)
    ...
    const description =
      extractSection(contextMd, 'What This Is') ||
      extractSection(contextMd, 'About') ||
      null
    ...
    const stack = stackRaw
      ...
      .filter(Boolean)
    const goal =
      (extractSection(contextMd, 'Current Goal') || '').split('\n')[0].trim() ||
      null
    ...
    const blockers = parseBullets(blockersRaw).filter(
      (b) => b.toLowerCase() !== 'none',
    )
    ...
    console.log(
      `    ✓ Migrated — ${sessions.length} session(s), ${decisions.length} decision(s)`,
    )
    migrated++
  } catch (e) {
